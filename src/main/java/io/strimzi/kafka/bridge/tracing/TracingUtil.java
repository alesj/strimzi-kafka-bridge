...
import static io.strimzi.kafka.bridge.tracing.TracingConstants.JAEGER_OPENTELEMETRY;
...
public class TracingUtil {
    ...
    public static void initialize(BridgeConfig config) {
        ...
        if (tracingConfig != null && (tracingConfig.equals(JAEGER_OPENTRACING) || tracingConfig.equals(JAEGER_OPENTELEMETRY))) {
            boolean isOpenTelemetry = JAEGER_OPENTELEMETRY.equals(tracingConfig);
            ...
            if (serviceName != null) {
                ...
                log.error("Jaeger tracingConfig cannot be initialized because {} environment variable is not defined", instance.envName());
            }
        }
    }
